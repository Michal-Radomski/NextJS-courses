...
async function addProperty(formData: FormData): Promise<void> {
  ...
  const images = formData.getAll("images").filter((image) => (image as File).name !== "") as File[];
  ...
  for (const imageFile of images) {
    ...
    const imageArray: number[] = Array.from(new Uint8Array(imageBuffer));
    const imageData: Buffer = Buffer.from(imageArray);
