...
export async function getPosts(maxNumber?: string | number): Promise<Post[]> {
  let limitClause = "";
...
export async function updatePostLikeStatus(postId: string, userId: string): Promise<sql.RunResult> {
  const stmt = db.prepare(`
